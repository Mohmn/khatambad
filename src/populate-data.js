...
export default class PopluateData {
  ...
  async generateDataTasks(dataQueryFn, dataGeneratorFn) {
    ...
    if (remainingRows > 0) {
      promises.push(
        this.createBatchPromise(dataQueryFn, dataGeneratorFn, remainingRows),
      );
    }
  ...
  createBatchPromise(dataQueryFn, dataGeneratorFn) {
    return async () => {
      try {
        ...
        // await this.dbClient.query('ROLLBACK');
        console.error(
          "Error in batch operation, transaction rolled back:",
          err,
        );
        throw err;
      }
...
class PopluateDependentTables extends PopluateData {
  ...
  async getColumnNames() {
    try {
      const columnNames = await this.dbClient.query(
  ...
  async getForeignTableNames() {
    try {
      ...
      await Promise.all(
        columnNames.map((element) => {
          return this.dbClient
            ...
            .then((result) => {
              if (result) foreignKeys.add({ [element]: result });
            })
            .catch((err) => {
      ...
      columnNames.forEach((key) => {
        if (foreignKeys[key])
          orderedForeignKeysAndTheirTableName[key] =
            orderedForeignKeysAndTheirTableName[key];
      });
